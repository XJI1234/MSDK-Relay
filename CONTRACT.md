...
一个运行中的 Android Relay 实例只管理一台当前连接的遥控器链路及其一架飞行器。需要同时管理多架飞行器时，电脑端必须分别连接多个 Android Relay 实例；手机端不得在一个 Relay 内复用、混合或切换两架飞行器的任务状态。
...
| `android-foreground-service-adapter` | 将 Android 前台服务和通知适配到 `ForegroundServicePort` | 不启动网关、DJI、遥测、图传或航线业务 |
...
手机端必须把命令交给 DJI 航线任务模块。`wayline.start` 的 `ok: true` 只表示 DJI 已接受启动请求；手机端任务状态必须保持 `STARTING`，不得显示为“执行中”。只有收到并发布 `mission-phase.phase = ROUTE_EXECUTION_STARTED` 后，电脑端和手机端才可将该任务显示为“执行中”。

暂停、恢复和停止同样只能按各自 DJI 操作终态报告，不能由请求已发出推断任务阶段。`wayline.stop` 只停止当前航线任务，绝不隐含返航、降落、`flight.return-home` 或 `flight.land`；这些高风险飞行动作只能由各自独立且明确确认的命令触发。
