...
4. `CompositeTelemetrySource`：只原子读取设备、飞行、直播和航线四类快照，并把任一来源变化合并为统一通知。组合根以 `FlightControllerKey.KeyConnection` 为飞行事实有效性的唯一开关：它明确断开时立即使飞行遥测快照失效；该 Key 之后明确重连时，才要求飞行遥测适配器重新建立 `listen + 异步 getValue(callback)` 观察。断开前内存快照及其迟到回调不得在重连后伪装为当前飞行事实。
