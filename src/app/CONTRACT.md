...
1. `MainActivity`：只承载地址输入、启动/停止操作和状态展示；不直接调用 DJI 或 WebSocket。状态顺序必须是运行时、电脑连接、MSDK 生命周期、遥控器连接、对频状态、飞控连接。MSDK 状态行必须一对一显示 `SdkLifecycle` 的受限状态；遥控器、对频和飞控状态行必须一对一显示 `RemoteControllerKey.KeyConnection`、`RemoteControllerKey.KeyPairingStatus`、`FlightControllerKey.KeyConnection` 的受限状态。`ProductKey.KeyConnection` 不进入手机 UI；它仅作为原始诊断事实保留，绝不翻译为飞机物理在线。展示文字必须是操作员能读的中文，不得把 `ACTIVE`、`RECONNECT_WAIT` 等机器词直接画上，也不得把多个 DJI Key 合并为同一行。开始/停止对频按钮只反映 `device-connection` 的能力：飞控明确断开时允许开始对频，飞控已连接或未知时不允许。来自后台状态监听的界面重绘是可替代工作：`MainActivity` 必须通过只保留最新 `MobileRelayStatus` 的主线程分发器提交，任意时刻至多保留一个尚未执行的 UI 重绘任务；被较新快照取代的中间绘制可以丢弃，但已提交的最新状态不得丢失。分发器关闭后不得渲染任何已经排队的旧状态。这个合并只属于界面呈现，不得改变 `MobileRelayGraph` 的状态通知、MSDK Key 观察、遥测发布、DJI 命令或其回执。首次创建时必须在 `onCreate`、生命周期到达 `STARTED` 前 `attach` 权限适配器，并把该实例交给图。中继仍在启动或运行时，界面销毁必须保留图和适配器，恢复时必须 `rebind` 到新界面，不得再次 `attach`，也不得因此停止电脑会话。仅当中继已停止、失败或正在停止时，`onDestroy` 才关闭图和适配器。
2. `MobileRelayGraph`：只创建一级模块的真实实例、注册命令并集中释放资源；不重新实现模块业务规则，对频按钮条件必须来自 `DeviceConnection.capabilities()`。旧 RTMP `LiveStream` 与 WHIP `WhipLiveStream` 必须拥有各自的适配器、状态、命令处理器和关闭路径；任一链路的启动、停止、发布器或 DJI 失败不得调用或改变另一链路。生产 RTMP 注入 `DeviceConnection.streamOperations()`，仅与自身启动/停止串行；飞行、航线、配对和设置必须分别注入 `flightOperations()`、`waylineOperations()`、`pairingOperations()` 与 `settingsOperations()`，互不占用槽位。生产 RTMP 的 `StreamStartGate` 必须直接复用 `DeviceConnection.capabilities().canStreamVideo`，即同一快照中 `SdkAvailability.READY`、`AirLinkKey.KeyConnection` 和 `CameraKey.KeyConnection(LEFT_OR_MAIN)` 均明确连接；图传源 Key 的未知或断开必须在调用 MSDK 前拒绝开始。组合根唯一负责在两个命令处理器之前装配运行时图传互斥，拒绝的命令不得调用另一链路。每次设备状态变化还必须以 `canStreamVideo` 观察生产 RTMP 图传源：从可用到不可用时只调用 `stream.markSourceUnavailable()`，从不可用恢复时只清除本地边沿标记，绝不自动重启 RTMP，也绝不调用 WHIP。权限适配器由 Activity 持有，图关闭时不得关闭它。
   飞行遥测订阅还必须把连续快照中的关键降落事实变化写入现有 `DiagnosticJournal`，事件模块为 `telemetry`、事件码为 `FLIGHT_STATE_CHANGED`。事件详情必须保留 `sourceGeneration`、`sourceRevision` 以及以下 MSDK 原始键名和值：`FlightControllerKey.KeyIsFlying`、`FlightControllerKey.KeyAreMotorsOn`、`FlightControllerKey.KeyFCFlightMode`、`FlightControllerKey.KeyIsLandingConfirmationNeeded`、`FlightAssistantKey.KeyLandingProtectionState`、`FlightControllerKey.KeyAltitude`。只记录值变化；高度仅在首次取得、变为未知或相邻值变化至少 0.5 米时记录，避免高频遥测淘汰命令证据。该记录器只消费 `FlightTelemetrySource` 已接受的快照，不读取 MSDK、不调用 DJI、不改变门禁或命令结果；中继诊断发布器负责按既有确认机制发送到桌面。
  航线订阅还必须把连续 `MissionSnapshot` 中的执行态、当前航点和中断原因写入同一 `DiagnosticJournal`，事件模块为 `wayline-mission`、事件码为 `WAYLINE_STATE_CHANGED`（INFO）与 `WAYLINE_INTERRUPT`（WARN）。详情只保留 `missionRevision`、`deviceGeneration` 以及上述字段的旧值到新值，中断另记受限 `djiErrorCode`/`djiErrorDescription`；不得记录文件名、路径或 KMZ。该记录器只消费 `WaylineMission` 已接受的快照。组合根还必须把 `AndroidDjiWaylineAdapter` 的观察诊断（含启动隔离丢弃的 `EXECUTION_STATE_OBSERVED`、`WAYLINE_PROGRESS_OBSERVED`、`WAYLINE_INTERRUPT_OBSERVED`）写入同一期刊，使断连后仍能解释 DJI 执行态与中断原因。
3. `RelayBootstrapModule`：只执行设备、遥测和网关的有序启停，隔离启动代次。SDK 离开 `READY` 或中继主动停止时，必须使直播、航线、飞行控制和设备设置失效，取消未终态的设备操作；主动停止不得把上一次 DJI 会话的本地业务状态带入下一次启动。网关已是 `ACTIVE` 但遥测尚未启动时发布设备链路快照。每次网关从非 `ACTIVE` 进入新的 `ACTIVE` 会话时，必须仅重置遥测发布去重基线、随后发布一份完整当前快照，不得为此重建 MSDK Key 观察。网关从 `ACTIVE` 进入重连或停止时只通知直播失效，从而停止 WHIP/RTMP 发布并释放图传互斥，不得因此停止网关重连。停止路径的生命周期锁只保护本模块状态和资源所有权；失效通知、端口停止调用及可能等待正在执行回调的监听注销都必须在锁外完成，避免停止中继、变更地址或销毁界面时发生锁反转。`MobileRelayGraph` 装配网关时握手超时必须为 15 秒，与桌面端一致。
...
组合根注册 `telemetry.read`、`pairing.start`、`pairing.stop`、`pairing.status`、旧 `live-stream.start|stop`、新 `live-stream-webrtc.start|stop`、全部 `wayline.*` 命令、`flight.takeoff`、`flight.land`、`flight.confirm-landing`、`flight.return-home`、`flight.stop-takeoff`、`flight.stop-auto-landing`、`device.settings.camera.read`、`device.settings.camera.write`、`device.settings.transmission.read` 和 `device.settings.transmission.write`。两条停止飞行动作与其余飞控命令同样要求 `{ confirm: true }`，只能经飞行域 DJI 操作协调器调用 `FlightControllerKey.KeyStopTakeoff` 或 `FlightControllerKey.KeyStopAutoLanding`。遥测快照必须完整映射为协议 `TelemetryFrame`，可选值缺失时使用 JSON null，不伪造零值。任何结果不得泄露密钥、路径或原始异常。会话状态变化和结束原因（含握手超时）同样写入 `DiagnosticJournal`，详情只用固定安全说明。

所有已注册命令只能经 `CommandDiagnosticRecorder` 包装一次。该记录器是观察者：它不得改变命令名、字段、执行顺序、DJI 调用、`CommandCompletion` 的 `ok`、`detail` 或可选结构化 `result`。除 `telemetry.read` 的成功请求和成功终态外，每个命令至少写入一条 `*_REQUESTED` 请求事件和恰好一条终态事件；终态事件为既有 `*_OK` / `*_REJECTED`、`*_HANDLER_FAILURE` 或 `*_DUPLICATE_RESULT`。请求和终态必须使用同一 `operationId`；终态详情必须包含 `phase`、终态类别和从本地单调时钟取得的 `elapsedMs`。当 `command-result.result` 是对象时，记录器只能白名单提取 `domain`、`outcome`；当且仅当 `outcome=ACTION_REJECTED` 时，才可额外记录原始 `errorCode` 和 `errorDescription`，并标记为 `djiErrorCode`、`djiErrorDescription`。不得序列化完整 JSON、命令字段、RTMP/WHIP URL、密钥、航线内容、文件路径、异常消息或堆栈。`RESULT_UNCONFIRMED`、`INVOCATION_FAILED` 和缺失结果必须如实记录，不能标为 DJI 拒绝。重复终态回调只写一条 `*_DUPLICATE_RESULT` 并不得再次完成命令；处理器同步抛出时必须写一条 `*_HANDLER_FAILURE`，随后以既有安全文本 `Command failed` 拒绝命令，且不得泄露异常内容。诊断写入失败不得影响命令执行或回执。

`telemetry.read` 只读取并发布当前持续 MSDK Key 观察已写入的完整快照；它不重建遥控器、产品、AirLink、相机、飞控或对频 Key 观察，不轮询桌面缓存，也不调用任何 DJI、飞控、航线或图传操作。只有 `device-connection` 在 SDK 已 `READY` 后才能建立 Key 观察：每个 Key 在观察建立时先注册 `KeyManager.listen`，再以带回调的 `KeyManager.getValue(key, callback)` 取得首次硬件值；每个后续 Key 变化由 `Telemetry` 对设备观察事件的订阅立即推送到电脑。配对命令使用 30 秒超时并且每条命令恰好产生一个终态。`pairing.status` 成功时必须通过 `command-result.result` 返回根契约 §7.4 的结构化快照：`pairingState`、`aircraftConnected`、`flightControllerConnected`、`aircraftModel`、`motorsOn`、`sdkRegistered`。该结果只来自当前遥测快照；遥测不可用时拒绝命令。`pairing.start` / `pairing.stop` 不得附带该结构化 `result`。旧 RTMP 直播和 WHIP 直播分别复用 `LiveStream` 与 `WhipLiveStream` 门面提供的处理器；旧 `live-stream.*` 命令不得路由到 WHIP，`live-stream-webrtc.*` 命令不得路由到旧 RTMP。航线、飞行控制和设备设置命令直接复用各一级模块门面提供的处理器。飞行控制处理器要求每次命令都带 `confirm: true`；设备设置处理器在读写成功时通过 `command-result.result` 返回完整结构化快照。四类 DJI 业务处理器都只能经 `device-connection` 各自对应的操作域协调器调用 DJI（航线 `waylineOperations()`、飞行 `flightOperations()`、设置 `settingsOperations()`、图传 `streamOperations()`），不得由组合根另建执行路径，也不得跨域占用槽位。

`wayline.start` 的手机端只由 `WaylineMission` / `MissionExecutor` 校验本地业务不变量：命令确认有效、任务身份来自已暂存文件、任务已成功上传、阶段转换合法，以及同设备操作串行化。它不得用遥控器、飞控、电量、飞行状态、电机或能力快照在本地替 DJI 预先裁决；`ProductKey.KeyConnection` 仅作诊断事实。满足业务条件后必须调用 DJI 航线启动 API，并把 `startMission` 的 `IDJIError` 以受限错误码和说明返回；无回调、超时、断连或调用异常只能报告未确认/调用失败，不得伪装成 DJI 拒绝。对频不是航线前置条件。桌面 `PreflightCheck` 只负责 Relay/MSDK 可达性及已上传阶段，不得添加第二套设备安全门禁。
