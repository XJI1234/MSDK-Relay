...
`TransportListener.onBytes` 是来自网络的高频入口。为了让一个变慢的命令处理器、任务暂存写入或依赖回调不能无限占用进程内存，会话执行器对已经复制、但尚未开始处理的入站字节帧施加以下固定规则：

- 最多允许 16 条等待中的入站字节帧；当前正在处理的一条不计入该上限；
- 只有取得等待名额的帧才可由会话保留其复制字节或处理闭包；被拒绝帧不得进入任何会话队列；
- 已获接收的普通入站帧仍按 transport 提供的到达顺序处理；显式 `stop()`、当前代次的 transport 关闭/失败、握手超时和入站溢出是终态会话控制事件，必须排到尚未开始的入站帧之前。它们不能中断一条已经交给 consumer 的帧，但该帧返回后必须先结束代次并丢弃所有等待帧；因此用户停止或传输断开不得等待一整段排队航线分块，也不得在断开后继续交付旧会话等待帧。普通 `start` 不享有此优先级；
- 第 17 条等待帧到达时，模块不再接收后续帧，而是在当前处理结束后优先结束该代次；结束原因为 `NOT_CONNECTED`，短消息固定为 `Inbound frame queue overflow`；
- 结束该代次时，所有尚未开始的入站帧必须被丢弃，不交给协议解码或业务模块；随后沿既有 `RECONNECT_WAIT` 退避规则重连；
- 该溢出不是协议帧错误，不得构造业务命令失败结果或伪造 DJI、航线、图传状态；电脑端会依既有连接关闭语义清理其未确认操作。

这项背压只限制未经处理的网络输入。它不改变已开始处理的帧、不重排正常业务帧，也不为任何 DJI、媒体或任务操作添加重试和门禁。
...
| 等待中的入站帧超过 16 条 | `NOT_CONNECTED` | 丢弃未开始帧，结束当前代次并按策略重连 |
...
- 一个变慢的入站帧处理器下，连续第 17 条等待帧结束当前代次、丢弃未开始帧并安排重连；
