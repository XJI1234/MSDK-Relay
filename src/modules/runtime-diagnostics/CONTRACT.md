...
它是观察者，不是控制者：诊断模块不得改变 DJI、连接、遥测、图传、航线或 Android 生命周期的业务决定。任何诊断记录、落盘、编码、发送和确认失败，都不得使被观察的业务失败、阻塞或重试；特别是 Android 文件 I/O 必须由有界后台写入者执行，不得在 MSDK、WebSocket、命令或 UI 回调线程同步重写完整快照。
...
- Android 适配器测试覆盖原子快照写入、进程恢复、Logcat/文件写入失败隔离、单后台写入者的最新快照合并、不记录敏感文本，以及调用 `record` 不等待实际文件写入。
