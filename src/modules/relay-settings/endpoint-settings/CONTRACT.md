...
失败只能是 `EMPTY`、`TOO_LONG`、`MALFORMED`、`INVALID_SCHEME`、`MISSING_HOST`、`INVALID_PORT`、`USER_INFO_NOT_ALLOWED`、`FRAGMENT_NOT_ALLOWED`、`CONTROL_CHARACTER` 和 `INVALID_PATH`。路径必须是 `/relay`；缺路径或只有 `/` 时补成 `/relay`。校验同步、纯函数、确定且线程安全。
