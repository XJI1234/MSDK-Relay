...
运行中的前台服务若被 Android 异常销毁并报告 `FAILED`，`AppRuntime` 必须先停止已启动的业务模块，再进入 `FAILED`；不得继续暴露 `RUNNING` 或留下孤儿模块。

停止请求在启动的每个阶段都必须有确定的收敛路径：等待权限时取消等待并进入 `STOPPED`；服务启动中时进入 `STOPPING`，等待该次服务启动的终态后直接停止服务且不得启动业务模块；模块启动中时进入 `STOPPING`，待该轮启动返回后逆序停止已启动模块，再停止服务。`STOPPING` 只能在服务实际停止或已经无法运行后变为 `STOPPED`，界面不得把过渡结果写成已停止。不得为了抢占服务启动而同时向 Android 服务提交相互竞争的开始/停止调用；旧启动回调只能驱动该已请求的停止收敛。
