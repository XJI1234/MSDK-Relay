...
设备不可用时，组合根必须调用 `LiveStream.markDeviceUnavailable`。门面将该通知唯一委托给 `StreamStateStore.markDeviceUnavailable`，使运行中图传进入安全非活动状态，并让旧 DJI 回调因操作代际失效而无法恢复图传。门面不得自行解释 RTMP URL、状态迁移或 DJI 错误。
