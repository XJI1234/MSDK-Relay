...
本模块经注入的图传 `DjiOperationCoordinator` 将已校验图传启动/停止请求适配到 DJI 图传 SDK，并将终态 DJI 结果转换为 `stream-state-store` 迁移。它不校验 URL、不解释中继命令、不持有图传事实、不发布遥测、不管理 WebSocket，也不创建执行器或调度器。
...
运行时错误、停止失败和设备不可用时的残余图传清理也只能作为同一图传协调器中的 `stop` 操作提交。适配器不得使用端口旁路直接调用 DJI。若启动在超时或取消后才报告成功，或随后才收到该代次的 DJI 状态/运行失败事实，适配器必须先用对应 `OperationCompletion.confirmHardwareSettled()` 释放协调器槽位，再排入一次清理停止；超时本身不能直接并发停止。

若运行期失败早于启动终态回执，适配器只能记录待恢复意图，不能把停止操作排在可能被超时隔离取消的队列中；启动成功/失败终态、迟到回调或超时后的权威确认释放槽位后，才允许提交该清理停止。

测试必须覆盖成功启动/停止、指标转发、重复完成、启动/停止前置条件拒绝、适配器异常、协调器拒绝、超时、取消、延迟回调和图传协调器串行操作。
