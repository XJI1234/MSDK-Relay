...
启动只可从 STOPPED/FAILED 进入 STARTING，并清空 `djiStreaming`；开始成功进入 STREAMING 但仍保持 `djiStreaming=null`，直到 `reportDjiStreaming` 收到 MSDK 的 `true`。`reportDjiStopped` 只接受当前 STREAMING 代次，记录 `djiStreaming=false` 并进入 FAILED；其他运行期失败、停止成功、设备不可用和新代次开始均将 `djiStreaming` 置为 `null`，不得伪造 false。停止只可从 STARTING/STREAMING 进入 STOPPING；匹配成功进入 STOPPED。过期或重复回调返回 `IgnoredStale`，不变更且不通知。无效指标必须在状态改变前抛出 `IllegalArgumentException`，无效请求返回稳定枚举原因。
