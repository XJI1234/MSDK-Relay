...
当前实机对照档固定使用 `LiveStreamType.RTMP`、主相机 `LEFT_OR_MAIN`、`StreamQuality.FULL_HD`（目标约 1920×1080）与 `LiveVideoBitrateMode.MANUAL`（500 KByte/s，约 4 Mbit/s）。这只影响下一次手动开始图传时 DJI 收到的配置，用于实机比较清晰度、延迟与卡顿；实际接受的分辨率、帧率、码率和 RTT 必须以 DJI 状态回报为准。分辨率仅在宽高均为正数时输出 `宽x高`；FPS、DJI 5.17 定义的 kbps 和 RTT 毫秒值仅在非负时输出。调用方异常必须隔离。
