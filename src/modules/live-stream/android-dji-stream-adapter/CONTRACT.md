...
每个开始或停止调用必须至多完成一次。DJI 同步异常和失败回调统一映射为 `completion.fail()`。成功开始后，`LiveStreamStatusListener` 的运行期错误或 `isStreaming=false` 都调用该代次的 `runtimeFailure`；开始完成前的 `false` 只是启动前基线，不得误报失败。停止、失败或新开始后到达的旧指标和旧错误必须忽略。
