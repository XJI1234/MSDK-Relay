...
`flight-control` 负责把桌面端的高风险飞行命令安全地提交给 DJI 飞控，并仅在 DJI 明确完成后返回终态结果。它支持 `flight.takeoff`、`flight.land`、`flight.return-home` 三个命令。
...
成功仅表示 DJI 已确认接收并完成对应动作调用；实际飞行状态由遥测模块独立上报。失败、超时、取消、设备不可用、重复或延迟回调均至多生成一条不含 SDK 错误、密钥、路径或异常详情的失败结果。
