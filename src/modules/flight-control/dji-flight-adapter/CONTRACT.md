...
把一个 `FlightAction` 作为飞行域 DJI 操作协调器中的单个操作执行，并映射为稳定终态。它拥有超时和取消语义，但不解析协议、不决定用户确认，也不依赖 Android 或 DJI SDK 类型。
