...
`beginTemporary` 必须关闭并清理旧临时文件；`append` 仅在活动写入期间有效；`flush` 必须刷新并同步文件描述符；`replaceCurrent` 必须先关闭临时流，再替换当前文件。实现优先保证同目录替换：新文件替换失败时必须恢复旧当前文件，避免丢失最后一份有效航线。读取只接受最后成功替换的同一元数据，返回字节副本。所有路径均由模块生成，外部文件名不得参与目录解析。
