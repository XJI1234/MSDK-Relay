...
启动命令在已接受后收到失败、超时、取消或适配器异常，不能证明 DJI 没有执行，因此必须保持 `STARTING`，禁止重发启动。暂停或继续在回执不确定时保持其已知执行状态；只有同一任务代际的匹配 DJI 状态观察到预期状态，才能同时解除本模块不确定性和共享 DJI 操作协调器的硬件槽位。无此确认时，停止也不能绕过共享槽位发送，以免与仍在执行的 DJI 调用重叠；界面必须如实显示命令未确认。后到的超时不得重新加锁。停止在超时或取消时保持 `STOPPING`，不得重发；明确失败或提交被拒绝时才恢复命令前状态。所有这些请求仍只向调用者报告一次安全终态，绝不把不确定性伪装成成功或失败。
