...
启动要求当前文件、上传状态 `UPLOADED` 和执行状态 `NOT_STARTED`/`FAILED`；暂停要求经阶段模块确认的 `EXECUTING`；继续要求 `PAUSED`；停止要求 `STARTING`/`EXECUTING`/`PAUSED`。同时最多一个控制操作。接受只表示提交：启动提交前记录 `STARTING`，`startMission` 成功后只完成启动命令，保持 `STARTING`，并由组合层武装 `mission-flight-phase`；只有门面接收阶段模块的 `ROUTE_EXECUTION_STARTED` 后才能原子写入 `EXECUTING`。暂停成功后 `PAUSED`；继续命令成功后恢复为 `EXECUTING`，但它不是新的首航点进入，绝不产生 `START_POINT_REACHED` 或 `ROUTE_EXECUTION_STARTED`；停止提交前 `STOPPING`、成功后 `FINISHED`。操作失败、超时、取消、适配器异常或协调器拒绝均记录 `FAILED`，仅前置条件拒绝保持原状态。
