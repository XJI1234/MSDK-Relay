...
状态：阶段事件与执行状态交接待按此契约修正；版本：2.2.0；所属一级模块：wayline-mission；Gradle 路径：:wayline-mission:mission-state-store
