...
`WaylinePlan`：安全 `.kmz` 基名 1..128 个 Unicode 码点、1..10,000 个有序航点、有限速度 0.1..15.0 m/s、受支持的 DJI 飞行器/负载声明、有限安全起飞高度和 `SAFELY` 入场模式；航点经度 -180..180、纬度 -90..90、有限高度 0..10,000 米。安全起飞高度相对起飞点：遥控器任务为 1.2..1500 米，机场任务为 8..1500 米。当前业务只接受 `SAFELY`，不开放 `POINT_TO_POINT`。`Generated` 含文件名、完整 KMZ 字节而非路径、小写 64 字符 SHA-256 和精确字节数。
