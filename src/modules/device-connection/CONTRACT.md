...
- 本模块拥有两个长期协调域：`operations()` 供配对、航线、飞行控制和设备设置使用，确保控制命令严格串行；`streamOperations()` 仅供生产 RTMP 图传使用，确保图传启动/停止严格串行。两个域可以独立等待 DJI 回调，符合图传与控制异步的链路边界；各模块不得自行创建 DJI 操作线程或绕开所注入协调器并发调用 DJI。
