...
`device-connection` 是手机端对 DJI MSDK 设备事实的唯一来源，也是 DJI 操作队列的唯一拥有者。它向其他一级模块提供不可变设备快照、设备能力和受控操作调度，但不实现遥测格式、航线业务、直播业务、WebSocket 或 Android 页面。
...
DeviceConnection.pairingOperations().submit(action, timeoutMillis, completion)
  -> OperationAccepted(cancellation) | OperationRejected
DeviceConnection.flightOperations().submit(action, timeoutMillis, completion)
  -> OperationAccepted(cancellation) | OperationRejected
DeviceConnection.waylineOperations().submit(action, timeoutMillis, completion)
  -> OperationAccepted(cancellation) | OperationRejected
DeviceConnection.settingsOperations().submit(action, timeoutMillis, completion)
  -> OperationAccepted(cancellation) | OperationRejected
DeviceConnection.streamOperations().submit(action, timeoutMillis, completion)
  -> OperationAccepted(cancellation) | OperationRejected
```
...
| `dji-operation-coordinator` | 对一个明确 DJI 操作域做串行、超时、取消和一次性完成保护 | `DjiOperationCoordinator` | 判断某项业务是否应执行 |
...
- 本模块拥有五个长期协调域：`pairingOperations()`、`flightOperations()`、`waylineOperations()`、`settingsOperations()` 与 `streamOperations()`。每个域只串行本域的 DJI 写操作（超时、取消、未确认隔离与 `CONTAINMENT` 收尾）。域与域之间不得互占槽位、不得因一个域超时而拒绝另一个域的提交。按钮对应一次 MSDK 调用，对错由该次回调和对应监听决定，不得再维护「全机正在执行哪条命令」。各模块不得自行创建 DJI 操作线程或绕开所注入协调器并发调用同一域的 DJI。状态监听和 RTMP 媒体数据不属于这些写操作队列。
