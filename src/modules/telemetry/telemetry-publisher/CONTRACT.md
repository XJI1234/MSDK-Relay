...
本模块只负责把已经进入 telemetry 后台发布出口的遥测快照按明确的去重规则交给发布端。它不修改设备状态、不编码协议、不管理 WebSocket、不读取 DJI。组合根将快照映射为实际 `TelemetryFrame` 时，必须为每个尝试发送的帧附加单调传输序号；该序号不属于本模块，也不是 DJI 事实。
...
- 不创建后台线程、定时器或隐式重试；调用方决定何时触发。一级 `telemetry` 模块负责确保此方法不从 DJI/Android 状态回调中同步调用。
