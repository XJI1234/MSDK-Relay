...
- 飞行状态、电机状态、飞行模式、低电量返航状态与预估时间、高度和位置来自 `FlightTelemetrySnapshot` 的飞控事实；电池连接和电量来自同一快照的独立电池事实。未收到值保持 `null`；MSDK 明确返回的飞行模式或低电量返航 `UNKNOWN` 必须原样透传。返航预估只允许返航状态不是 `UNKNOWN` 的正秒数；纬度和经度必须同时存在且在合法范围内，否则组装前拒绝输入。
- 当 `DeviceSnapshot.flightController` 为 `DISCONNECTED` 时，飞行源中的最近飞控值已经不能代表当前设备事实；输出必须将飞控动态字段（飞行状态、电机、模式、低电量返航状态与预估时间、高度和位置）置为 `null`。不得因此清空电池连接或电量。`BatteryKey.KeyConnection` 为 `CONNECTED` 时可继续输出有效电量；为 `DISCONNECTED` 或 `UNKNOWN` 时电量必须为 `null`。重新连接并取得新快照后才可再次输出飞控字段。
