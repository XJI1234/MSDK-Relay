...
- 飞行状态、电机状态、飞行模式、电量、剩余飞行时间、高度和位置来自 `FlightTelemetrySnapshot`；未知值保持 `null`。纬度和经度必须同时存在且在合法范围内，否则组装前拒绝输入。
- 当 `DeviceSnapshot.flightController` 为 `DISCONNECTED` 时，飞行源中的最近值已经不能代表当前设备事实；输出必须将全部飞行动态字段（飞行状态、电机、模式、电量、剩余时间、高度和位置）置为 `null`。重新连接并取得新快照后才可再次输出这些字段。
- 直播状态和指标来自 `StreamSnapshot`；航线执行状态、上传进度和文件名来自 `MissionSnapshot`。上传进度仅在上传中暴露 0..100，已上传时为 100，其他状态为空。
