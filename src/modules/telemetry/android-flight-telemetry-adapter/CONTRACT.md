...
- 平台释放失败必须被隔离；逻辑代次仍立即失效。飞控与电池监听的释放必须分别尝试，任一 DJI `cancelListen` 抛错不得阻止另一监听器的释放。
